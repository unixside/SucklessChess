...
import { getEnemyColor, getKingChar, getPieceColor } from '../utils/functions';
...
export class GameStateImpl implements IGameState {
    ...
    public findValidCastlings(color: Color): CastlingSetup[] {
        ...
        for (const rookSquare of rooks) {
            ...
            const setup = this.createCastlingSetup(kingSquare, rookSquare);
    ...
    private hasPieceMoved(color: Color): boolean {
        ...
        if (color === 'w') {
            if (rooks.some((r) => r === 'h1') && !this.availableCastlings.K)
                return true;
            if (rooks.some((r) => r === 'a1') && !this.availableCastlings.Q)
                return true;
        } else {
            if (rooks.some((r) => r === 'h8') && !this.availableCastlings.k)
                return true;
            if (rooks.some((r) => r === 'a8') && !this.availableCastlings.q)
                return true;
        }
        return false;
    }
