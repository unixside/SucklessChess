import { Castlings, Color, Rows } from '../models/types';
...
export namespace FenValidations {
    function validateWithRegExp(
        ...
        if (value === undefined || value === null || value === '') {
            return Fail('Value is undefined, null or empty string');
        }

        if (!regexp.test(value)) return Fail(`Invalid syntax for ${name}`);
    ...
    export function piecePlacement(value?: string): Result<string> {
        ...
        if (!ChessRegExp.piecePlacement.test(value)) {
            return Fail('Invalid character in value');
        }
        ...
        if (whiteKing?.length !== 1 || blackKing?.length !== 1) {
            return Fail('Invalid number of kings in piece placement');
        }

        let rows = value.split('/');

        if (rows.length !== 8) {
            return Fail('Invalid number of rows');
        }
        ...
        for (const row in rows) {
            ...
            if (number_spaces + number_pieces > 8) {
                return Fail('Invalid number of spaces or pieces in a row');
            }
        }
    ...
    export function fen(value: string): Result<IGameState> {
        if (value === undefined || value === null || value === '') {
            return Fail('Value is undefined, null or empty string');
        }
        ...
        let board: Board = new Board(piece_placement);
        let castlings: Castlings = {
            Q: available_castlings.includes('Q'),
            K: available_castlings.includes('K'),
            q: available_castlings.includes('q'),
            k: available_castlings.includes('k'),
        };
        ...
        return Ok({
            board: board,
            activeColor: active_color as Color,
